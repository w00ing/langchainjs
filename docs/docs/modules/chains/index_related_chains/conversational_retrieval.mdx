...
static fromLLM(
  ...
  options: {
    ...
    qaTemplate?: string;
    chainType?: string; // default is 'stuff_documents_chain'; you can set it to 'map_reduce_documents_chain'.
  } = {}
): ConversationalRetrievalQAChain
```
...
- `chainType`: A string that specifies the type of chain to use for the question answering step. The default value is `stuff_documents_chain`, which is a chain that uses the `StuffDocumentsChain` class. You can also set this to `map_reduce_documents_chain`, which is a chain that uses the `MapReduceDocumentsChain` class.

In summary, the `questionGeneratorTemplate`, `qaTemplate`, `returnSourceDocuments` and `chainType` options allow the user to customize the behavior of the `ConversationalRetrievalQAChain`
